...
var Module = (function(CONFIG) {
    ...
    function initConfiguration() {
        ...
        CONFIG.DELAY_DURATION = parseInt(CONFIG.DELAY_DURATION) * 1 || 3 ;
    ...
    function main() {
        ...
        if (! initConfiguration()) {
            throw new Error("The script configuration could not be initialized.");
            return;
        }
    ...
    function doLoadMetaData() {
        ...
        try {
            ...
            new File(CONFIG.COMMANDER_PATH).execute();            

            // 3. Open META.json

            $.sleep(CONFIG.DELAY_DURATION * 500);

            return Utils.read_json_file(
                $HERE + "tmp/META-" + UUID + ".json"
            );
        }
        catch(ex) {
            logger.error(localize({en_US: ex.message}));
            // $.write($.stack);
        }
    };
    ...
    function copyMetaFile() {
        try {
            ...
            new File(CONFIG.META_GZ_FILE).copy(
                $HERE + "tmp/META-" + UUID + ".zip"
            );
        }
        catch(ex) {
            throw new Error(ex);
